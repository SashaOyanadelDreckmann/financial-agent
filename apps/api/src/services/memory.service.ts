...
const SYSTEM_MEMORY_PATH = path.join(MEMORY_DIR, 'system.json');
...
function userMemoryPath(userId: string) {
...
export function loadSystemMemory(): SystemMemory {
  ...
  const payload = buildSystemMemoryPayload();
  atomicWriteJson(SYSTEM_MEMORY_PATH, payload);
  return payload;
}

export function loadUserMemory(userId: string): UserMemory {
  ensureDir(USER_MEMORY_DIR);
  const filePath = userMemoryPath(userId);
...
export function saveUserMemory(memory: UserMemory): UserMemory {
  ...
  atomicWriteJson(userMemoryPath(memory.userId), memory);
  return memory;
}
...
export function appendTurnToMemory(params: {
  ...
  const budgetSummary = (input.ui_state?.budget_summary ?? {}) as Record<string, any>;
  const contextBudget = (input.context?.budget_summary ?? {}) as Record<string, any>;
  const mergedBudget = {
