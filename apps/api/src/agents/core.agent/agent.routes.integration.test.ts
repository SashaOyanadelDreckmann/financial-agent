 ...
 * Tests HTTP endpoints, validation, rate limiting, response formats
 */
...
describe('Agent API Routes Integration', () => {
  ...
  it('POST /api/agent with valid input returns 200 with ChatAgentResponse', async () => {
    // Expected flow:
    // 1. POST /api/agent with ChatAgentInput
    // 2. Validate request body against ChatAgentInputSchema
    // 3. Call runCoreAgent(input)
    // 4. Return 200 with ChatAgentResponse
    // 5. Response includes: message, mode, tool_calls, react, artifacts, compliance, meta
    ...
    // - status: 200
    // - body.message: string
    // - body.mode: ReasoningMode
    // - body.meta.turn_id: UUID
    // - body.meta.latency_ms: number > 0
    // - body.compliance.no_auto_execution: true

    expect(validInput.user_message).toBeTruthy();
  });

  it('POST /api/agent with missing required fields returns 400', async () => {
    // Expected flow:
    // 1. POST /api/agent without user_message
    // 2. Validation fails
    // 3. Return 400 with validation_error message
    // 4. Body includes: { error: "VALIDATION_ERROR", issues: [...] }
    ...
    // - body.issues[0].path: ["user_message"]
    // - body.issues[0].message: "Required"

    expect(invalidInput).not.toHaveProperty('user_message');
  });

  it('POST /api/agent without auth token returns 401', async () => {
    // Expected flow:
    // 1. POST /api/agent without Authorization header
    // 2. Auth middleware checks for valid token
    // 3. Return 401 Unauthorized
    // 4. Body includes: { error: "UNAUTHORIZED" }
  ...
  it('POST /api/agent with invalid JSON returns 400', async () => {
    // Expected flow:
    // 1. POST /api/agent with malformed JSON
    // 2. JSON parsing fails
    // 3. Express error handler catches SyntaxError
    // 4. Return 400 with parse error

    // Assertions:
    // - status: 400
    // - body.error includes "JSON"
