...
describe('runFormatPhase', () => {
  ...
  it('should generate formatted response from LLM output', async () => {
    ...
    (complete as any).mockResolvedValueOnce(mockResponse);
...
<CONTEXT_SCORE>80</CONTEXT_SCORE>`;
  ...
  it('should include artifacts from execution result', async () => {
    (complete as any).mockResolvedValueOnce('Response text');
...
invalid json
  ...
  it('should throw on LLM error', async () => {
    (complete as any).mockRejectedValueOnce(new Error('LLM service error'));
