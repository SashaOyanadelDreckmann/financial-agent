...
export default function IntakePage() {
  ...
  const [form, setForm] = useState<IntakeQuestionnaire>(
    structuredClone(INITIAL_FORM)
  );
  ...
  // Helpers
  // ──────────────────────────────
  const update = <K extends keyof IntakeQuestionnaire>(
    key: K,
  ...
  const onSubmit = async () => {
    try {
      ...
      setError(e.message ?? 'Error enviando el diagnóstico');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="w-full min-h-screen flex justify-center pt-24">
      ...
      {error && (
        <p className="fixed bottom-8 text-red-400 text-sm">
          {error}
        </p>
      )}
    </div>
